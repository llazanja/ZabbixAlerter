...
Client.prototype.login = function login(callback) {
  ...
  this.call('user.login', {
	...
	callback(error, response, body, this);
    }.bind(this)
	   );
};
...
Client.prototype.processItem = function processItem(hostname, key, callback) {
	this.call('item.get', {
					"output": [
						"itemid"
					],
...
Client.prototype.getHistory = function getHistory(id, type, timespan, callback) {
	this.call('history.get', {
					"output": "extend",
					"history": type,
					"itemids": id,
          "time_from": ((Date.now()  - timespan * 60 * 1000) / 1000),
					"sortfield": "clock",
					"sortorder": "DESC"
			}, function (error, response, body) {
				callback(error, response, body);
